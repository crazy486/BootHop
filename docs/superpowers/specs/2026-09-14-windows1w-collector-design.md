...
The maximum payload is 1 MiB per variable and the retained `BootOrder`/`BootCurrent`/`Boot####` enumeration evidence has a separate aggregate 1 MiB budget; `BootNext` is outside that aggregate. Buffer growth is bounded and only follows an explicit buffer-too-small outcome; exceeding either cap fails closed without truncated success. `BootOrder` must be an even-length little-endian `UINT16` array. `BootCurrent` and a successful `BootNext` must each be exactly one little-endian `UINT16`. Every successfully read referenced `Boot####` payload is retained separately from parsed `EFI_LOAD_OPTION` evidence, including payloads rejected for attributes or malformed parsing. Only validation- and parse-valid payloads become `OptionEvidence`; rejected raw captures remain non-accepting and are written to private exclusive files. All raw captures are bounded by the same aggregate budget and are parsed with `boothop-core` only for validation.
