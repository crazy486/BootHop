...
BootNext与BootCurrent必须**总长6字节**（4字节attributes+2字节u16 LE），不是8字节；BootOrder总长>=4且减4后为偶数，空数组可表示但不得自动选项，重复编号去重并诊断。`read_options` 只读取并验证 BootOrder、BootCurrent 和发现的 Boot####，不打开 BootNext；BootNext 仅由 Switch 的显式 `read_next` 读取和验证。因此 malformed 或 inaccessible BootNext 不得改变 Inspect/Configure 的选项枚举结果。BootNext不存在只有固定有效目录下该叶变量明确ENOENT可映射None；目标Boot#### ENOENT是目标缺失，枚举后消失不可当完整成功；记录NotFound仅在可信目录/路径检查完成后为RecordState::Missing。BootOrder/BootCurrent缺失需保留诊断，不推断BootNext能力；switch仍须原目标可读和所有实际存在控制变量结构/属性有效。
