...
单次选项枚举**保留raw的累计预算1 MiB**：所有保留的 BootOrder、BootCurrent 与 Boot#### inventory 原始buffer字节数之和（包含Linux4字节前缀），checked累计；不包含由 Switch `read_next` 单独读取的 BootNext。超限ResourceLimit且不返回部分完整列表。无必要副本及时丢弃；单项读取最多1 MiB+4的临时buffer不能成为绕过累计预算的永久缓存，内存分配失败同样ResourceLimit。这是controller确认的产品限制，不是UEFI上限，与单项payload1 MiB、单份记录1 MiB、IPC总帧/累计65,536字节是四个独立计数域；不将限制扩大为1 MiB IPC。测试 `enumeration_raw_aggregate_limit_no_partial_success` 与 `single_payload_record_ipc_budgets_are_distinct`。

BootNext与BootCurrent必须**总长6字节**（4字节attributes+2字节u16 LE），不是8字节；BootOrder总长>=4且减4后为偶数，空数组可表示但不得自动选项，重复编号去重并诊断。`read_options` 只读取并验证 BootOrder、BootCurrent 和发现的 Boot####，不打开 BootNext；BootNext 仅由 Switch 的显式 `read_next` 读取和验证，按独立的单项读取上限计数，不纳入上述枚举累计预算。因此 malformed 或 inaccessible BootNext 不得改变 Inspect/Configure 的选项枚举结果。BootNext不存在只有固定有效目录下该叶变量明确ENOENT可映射None；目标Boot#### ENOENT是目标缺失，枚举后消失不可当完整成功；记录NotFound仅在可信目录/路径检查完成后为RecordState::Missing。BootOrder/BootCurrent缺失需保留诊断，不推断BootNext能力；switch仍须原目标可读和所有实际存在控制变量结构/属性有效。
...
| Linux变量 | `efivar_prefix_and_payload_separate`、`boot_next_exact_six_bytes`、`ready_inspect_does_not_open_boot_next_during_option_enumeration`、`missing_record_inspect_does_not_open_boot_next_during_option_enumeration`、`malformed_boot_next_does_not_affect_inspect_inventory` | §3.2全部errno/长度测试；`bootorder_empty_not_auto_target`、`enumerated_target_disappears_fails`、`switch_explicitly_reads_boot_next_and_fails_closed_on_malformed_or_inaccessible_value` |
