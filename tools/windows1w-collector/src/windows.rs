...
impl WindowsCalls for WindowsBackend {
    ...
    fn enable_privilege(&mut self) -> Result<PrivilegeState, CallError> {
        ...
        if adjust_error == ERROR_NOT_ALL_ASSIGNED {
            return self.close_after_error(CallError::new(ERROR_NOT_ALL_ASSIGNED));
        }
        ...
        {
            return self.close_after_error(CallError::new(ERROR_INVALID_PARAMETER));
        }
    ...
    fn restore_privilege(&mut self, _state: PrivilegeState) -> Result<(), CallError> {
        let Some(previous) = self.previous_privilege.take() else {
        ...
        let adjustment = if restored == 0 || restore_error == ERROR_NOT_ALL_ASSIGNED {
    ...
    fn read_variable(&mut self, variable: VariableName, buffer_size: usize) -> ReadOutcome {
        ...
        if returned != 0 {
            ...
            return ReadOutcome::success(attributes, buffer);
        }
        ...
        if error == ERROR_ENVVAR_NOT_FOUND {
            return ReadOutcome::missing(error);
        }
        ReadOutcome {
