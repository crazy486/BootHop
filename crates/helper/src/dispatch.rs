    ...
    P: Platform,
{
    ...
    // The explicit finish keeps release ordering observable and propagates a
    // release failure even when the terminal response was already sent.
    let release = guard.finish();
    release.map(|()| outcome).unwrap_or_else(Err)
}
