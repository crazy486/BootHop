...
use boothop_gui::cache::{Cache, CachedTarget, LinuxCache, resolve_cache_path, safe_description};
use std::{
    ffi::OsStr,
    fs,
    path::PathBuf,
...
fn xdg_paths_require_an_absolute_trusted_base() {
    assert_eq!(
        resolve_cache_path(Some(OsStr::new("/tmp/state")), Some(OsStr::new("/ignored"))).unwrap(),
        PathBuf::from("/tmp/state/boothop/cache-v1.json")
    );
    assert_eq!(
        resolve_cache_path(Some(OsStr::new("relative")), Some(OsStr::new("/tmp/home"))).unwrap(),
